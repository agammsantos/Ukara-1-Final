...
data=pd.read_csv('data_train_A.csv')
print(len(data))
...
freqdata=pd.Series(' '.join(data['words']).lower().split(' ')).value_counts()[:10]
...
for i in groupedfix: # creating stopwords from both labels same common words
    freqdatalabel=pd.Series(' '.join(i[1]['words']).lower().split(' ')).value_counts()[:10]
...
test=pd.read_csv('data_test_A.csv')
...
# complementPrediksiTrain = complementPipeline.predict(data['RESPONSE'].apply(lambda x:x.lower()))
...
with open('../Data B/data1mix.json', 'w') as json_file:
